...
def predict(sample):
    ...
    # Return the prediction as a binary label
    if prediction == 0:
        return 'Poor Quality'
    else:
        return 'Good Quality'
...
if st.button("Predict"):
    ...
    fixed_acidity = st.slider("Fixed acidity:", 0, 1, 0.5)
    volatile_acidity = st.slider("Volatile acidity:", 0, 1, 0.5)
    citric_acid = st.slider("Citric acid:", 0, 1, 0.5)
    residual_sugar = st.slider("Residual sugar:", 0, 1, 0.5)
    chlorides = st.slider("Chlorides:", 0, 1, 0.5)
    free_sulfur_dioxide = st.slider("Free sulfur dioxide:", 0, 1, 0.5)
    total_sulfur_dioxide = st.slider("Total sulfur dioxide:", 0, 1, 0.5)
    density = st.slider("Density:", 0, 1, 0.5)
    pH = st.slider("pH:", 0, 1, 0.5)
    sulphates = st.slider("Sulphates:", 0, 1, 0.5)
    alcohol = st.slider("Alcohol:", 0, 1, 0.5)
    quality = st.text_input("Quality:", "low")

    # Process the input values and trigger the prediction
    # (your prediction code goes here)
    prediction = "High"  # placeholder for prediction
